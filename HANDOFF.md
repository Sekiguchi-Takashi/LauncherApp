...
## v3.3.2（バグ修正：編集モードでアイコンを移動できない）
- 原因: HorizontalPager の横スクロールがドラッグを奪っていた。長押しでジグルには入るが、動かそうとすると Pager がページ送りとして処理してしまい移動できなかった
- 修正
  - 編集モード中は `userScrollEnabled = !editMode` で Pager のスワイプを止める。ドラッグはすべてアイコン側が受け取る
  - ページ移動の手段を確保するため、ページドットをタップで移動できるようにした（当たり判定 20dp、見た目のドットは従来サイズ）。従来どおり、アイコンを左右端で離すページ間移動も使える
  - 編集モード中はタップでメニューを開けるようにした（`clickable { menuFor = item }` を drag 検出の外側に置く）。従来は drag が成立しないとメニューが出せなかった
  - ドラッグ中の浮遊アイコンに 14dp の影を付けて、掴んでいる感触を分かりやすくした
- 操作の流れ: アイコンを長押し → 揺れ始める → 指を離す → 好きなアイコンをそのままドラッグして移動 → 「完了」で終了

## v3.3.3（バグ修正：アイコンが少ししか動かない）
- 原因1: ドラッグ開始位置の計算が実際の指の位置とずれていた
  - 旧: `dragStart = Offset(c * cellW + offset.x, r * cellH + offset.y)`。offset はアイコン Column 内の座標なのに、セル左上を原点として足していた。Column はセルより小さく中央寄せなので、その差（横約20dp・縦約25dp）が丸ごと誤差になり、落とす先のセル判定が本来より手前に寄っていた
  - 新: BoxWithConstraints に onGloballyPositioned を付けてグリッド原点を実測し、各セルも positionInWindow から自分の位置を持つ。`dragStart = cellOrigin + offset` で誤差なし
- 原因2: 埋まったセルに落とすとフォルダ化されるため、移動にならなかった
  - 全アプリを自動配置している都合でセルはほぼ埋まっており、どこに落としてもフォルダになっていた
  - dropOnto を変更: アプリをアプリに落としたら「入れ替え」。フォルダに落としたときだけ中に追加、フォルダ同士は統合（従来どおり）
  - 代わりに makeFolder() を追加し、長押しメニューの「フォルダを作る」でその場にフォルダを作れるようにした。あとは他のアイコンをそのフォルダへ落として増やす

## v3.3.4（バグ修正：ドラッグ位置の追跡を絶対座標に変更）
- 症状の継続: v3.3.3 でも「揺れるが少ししか動かない」。スクリーンショットでは浮遊アイコン（ラベル・バッジ無し）がセルから外れた位置に見えており、ドラッグ量の積み上げ（dragPos += amount）がどこかで欠落して指と乖離していると判断
- 修正
  - onDrag で移動量を足し込むのをやめ、`dragPos = cellOrigin + change.position` と毎回指の絶対位置を実測する方式に変更。途中でイベントが欠落しても、最終的な dragPos は必ず指の真下になり、落とし先の判定も指の位置と一致する
  - 保険として、editMode が false になったら dragItem を null に戻す LaunchedEffect を追加（ドラッグ状態が残って浮遊アイコンが固まるのを防ぐ）
- 備考: change.position はセル Column のローカル座標。cellOrigin（グリッド原点との差、実測）と足すことでグリッド座標になる。ジグルの回転（±2.5度）による誤差は数px で無視できる

## v3.4 実装済み（移動方式をドラッグからタップ選択へ全面変更）
- ドラッグでの移動は断念し、コードごと撤去した
  - 削除: dragItem / dragStart / dragPos の状態、detectDragGestures によるセルのドラッグ、指に追従する浮遊アイコン、ページ端で離すページ間移動、WorkspacePage と HomeScreen の onMoveItem
  - dropOnto() は呼び出し元が無くなった（AppModel には残置）
- 新しい操作
  1. アイコンを長押し → 編集モード（全体が揺れる）
  2. 動かしたいアイコンをタップ → 選択状態（1.18倍に拡大＋やや半透明）
  3. 空いているマスをタップ → そこへ移動
  4. すでにアイコンがあるマスをタップ → 選択解除（移動しない）
  5. 何も選択していない状態で空きマスをタップ → 編集モード終了
  - 編集モード中のアイコン長押しは従来のメニュー（フォルダを作る / 名前を変更 / アイコンを変更 / App Libraryへ移動 / アプリ情報 / ショートカット）
